...
def eval_MSE_sol(individual, true_data):
    ...
    try:
        ...
        V_vals, V_grad_mag, V_dot_vals, lambda_vals, _, u_mag = compute_v_and_v_dot(V_vals, X1, X2)
        ...
        MSE = (1000000000 * V_violations + V_dot_violations + penalty + 1 * lmbda_penalty) + V_val_0**2 #+ lambda_grad_mag\
        #+ 100 * np.mean(u_mag) #+ 0.05 * np.mean(V_grad_mag)
