...
def eval_MSE_sol(individual, true_data):
    ...
    try:
        ...
        #
        lmbda_penalty = (np.count_nonzero(lambda_vals < 0.5) + np.count_nonzero(lambda_vals > 5)) #* 100
        ...
        MSE = (1000000000 * V_violations + V_dot_violations + penalty + V_val_0**2) + 0.05 * np.mean(V_grad_mag) + 1 * lmbda_penalty
         #+ lambda_grad_mag\
        #+ 100 * np.mean(u_mag) #
