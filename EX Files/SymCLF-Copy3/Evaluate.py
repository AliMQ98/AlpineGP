...
def eval_MSE_sol(individual, true_data):
    ...
    try:
        ...
        _, V_grad_mag, V_dot_vals, lambda_vals, u_vals, _, u_mag = compute_v_and_v_dot(V_vals, x1_vals, x2_vals)
        ...
        lmbda_penalty = 10* np.mean((lambda_vals - 1)**2) #+ np.std(lambda_vals)**2

        
        u_penalty = np.count_nonzero(u_mag > 25) * 1

        
        #lmbda_penalty += (np.count_nonzero(lambda_vals < 0) + np.count_nonzero(lambda_vals > 10)) #* 100
        ...
        lambda_grad_mag = np.mean(lambda_grad_mag) * 1
        ...
        MSE = (V_violations + V_dot_violations + penalty + V_val_0**2) + 0.1 * np.mean(V_grad_mag) + 0.1 * lmbda_penalty + lambda_grad_mag + u_penalty + u_grad_mag 
        #+ 100 * np.mean(u_mag) #
