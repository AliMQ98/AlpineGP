...
def eval_MSE_sol(individual, true_data):
    ...
    try:
        ...
        lmbda_penalty = 10* np.mean((lambda_vals - 50)**2) #+ np.std(lambda_vals)**2
        ...
        V_grad_mag = np.mean(V_grad_mag)
        V_grad_x1 = np.gradient(V_vals, x1_vals, axis=1)  # Gradient w.r.t x1  
        V_grad_x2 = np.gradient(V_vals, x2_vals, axis=0)
        V_grad_x2x1 = np.gradient(V_grad_x2, x1_vals, axis=1)
        V_grad_mag += np.count_nonzero(V_grad_x2x1**2 < 1)
        V_grad_mag += np.count_nonzero(V_grad_x1**2 < 1)
        V_grad_mag += np.count_nonzero(V_grad_x2**2 < 1)
        ...
        lambda_grad_mag = np.mean(lambda_grad_mag) * 10
        ...
        MSE = (V_violations + a_violations + V_dot_violations + penalty + 1000 * V_val_0**2) + 1 * V_grad_mag + 0.1 * lmbda_penalty + lambda_grad_mag
        #+ 0.1 * np.mean(V_grad_mag)  #+ u_penalty + u_grad_mag 
        #+ 100 * np.mean(u_mag) #
