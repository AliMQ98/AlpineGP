...
from alpine.gp import util  # Import utility functions from the alpine genetic programming module
import re  # Regular expressions for string processing
from sympy import sympify  # Symbolic computation library
...
def get_features_batch(
    ...
    individ_length = features_batch[0]  # Length of each individual expression
    nested_trigs = features_batch[1]    # Number of nested trigonometric functions
    num_trigs = features_batch[2]       # Total number of trigonometric functions
...
def DeapSimplifier(ind):
    ...
    """
    # Define custom operators to handle DEAP-style symbolic expressions
    locals = {
        'sub': lambda x, y: x - y,
        'div': lambda x, y: x / y,
        'mul': lambda x, y: x * y,
    ...
    # Convert the string expression into a sympy symbolic expression
